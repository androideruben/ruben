...
ruben <- data.frame(id=c(sort(persid)), firstnameB=c(first),
            lastnameB=c(last), stateB=c(stat))
rr<- data.frame(df2, ruben)


df3 <- merge("SELECT id, firstname
              FROM df2
              JOIN ruben USING(id)")
df3

#Outer join: 
all=merge(x = df2, y = ruben, by = "id", all = TRUE)
all

#Left outer: 
left=merge(x = df1, y = df2, by = "id", all.x = TRUE)
left

#Right outer: 
right=merge(x = df1, y = df2, by = "id", all.y = TRUE)
right

#Cross join: 
cross=merge(x = df1, y = df2, by = NULL)
cross
...
df2b=sqldf("SELECT COUNT(*) FROM df2 WHERE state = 'CA'")
df2b
