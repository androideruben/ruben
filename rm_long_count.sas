...
/***********************************************************************************************/
**http://support.sas.com/resources/papers/proceedings17/0202-2017.pdf;
**Longitudinal Count Data Using the NLMIXED Procedure;
/************************************************************************************************/

ods pdf file = "\\fda.gov\WODC\CTP_Sandbox\OS\DPHS\StatisticsBranch\Team 2\Montes de Oca\rm_EXPLORE\4-LMM\results\&program_name &sysdate9..pdf";
*ID Count Visit Trt Age Weeks;

PROC NLMIXED data=work.seizure;
parms logsig=0 beta0=1 beta1=1; 
lambda = exp(beta0 + beta1*trt + u);
model count ~ poisson(lambda);
random u~ normal(0,exp(2*logsig)) subject=ID;
RUN;

proc genmod data=work.seizure;
...
stop;
run;
data PMF; do t= 0 to 40;
Y = pdf("Poisson", t, &Lambda); output;
end;
run;
proc freq data=work.seizure;
...
title "Does 'Count' have a Poisson Distribution?";
run;
ods pdf close;
